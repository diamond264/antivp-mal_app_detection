...
class RecorderInCallService : InCallService(), RecorderThread.RecordingEventListener {
    companion object {
    ...
    private fun handleStateChange(call: Call, state: Int?) {
        ...
        when (callState) {
            ...
            Call.STATE_RINGING -> {
                CallEventGenerationModule.notifyRingingEvent(application, System.currentTimeMillis()/1000L,
                    "in", call.details.handle?.schemeSpecificPart?:"")
            }
        }
    }
    ...
    private fun createPersistentNotification(): Notification {
        ...
        return Notification.Builder(this, AntiVPApplication.CHANNEL_ID_PERSISTENT).run {
    ...
    private fun createRecordingFailedNotification(errorMsg: String?, uri: Uri?): Notification =
        Notification.Builder(this, AntiVPApplication.CHANNEL_ID_ALERTS).run {
    ...
    override fun onRecordingStarted(thread: RecorderThread, dialogueId: String) {
        Log.i(TAG, "Recording started: ${thread.id}: $dialogueId")
        val fileManager = FileManager.getInstance()


        fileManager.createDirectory("calls")
        fileManager.createDirectory("calls/${dialogueId}")



//        CallEventGenerationModule.notifyCallStartEvent(applicationContext, uri,
//            thread.callTimestamp.toEpochSecond(), thread.callDirection, thread.phoneNumber)
    }

    override fun onRecordingCompleted(thread: RecorderThread, uri: Uri) {
        Log.i(TAG, "Recording completed: ${thread.id}: ${thread.redact(uri)}")
        CallEventGenerationModule.notifyCallEndEvent(applicationContext, true, uri,
            thread.callTimestamp.toEpochSecond(), thread.callDirection, thread.phoneNumber)
    ...
    override fun onRecordingFailed(thread: RecorderThread, errorMsg: String?, uri: Uri?) {
        ...
        CallEventGenerationModule.notifyCallEndEvent(applicationContext,  false, uri,
            thread.callTimestamp.toEpochSecond(), thread.callDirection, thread.phoneNumber)
    ...
    override fun onSavingVoiceFile(thread: RecorderThread, fileName: String, dialogueId: String){
        Log.i(TAG, "Saving file: $fileName, $dialogueId")
        CallEventGenerationModule.notifyCallStartEvent(applicationContext, fileName, dialogueId)
    }
}