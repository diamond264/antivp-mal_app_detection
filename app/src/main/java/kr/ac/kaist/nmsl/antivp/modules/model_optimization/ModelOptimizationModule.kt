...
class ModelOptimizationModule : Module() {
    ...
    public var sttModel = null
    public var detectionModel = null

    private val sttPruningLevels = intArrayOf(0, 1, 2)
    private val detectionPruningLevels = intArrayOf(0, 1, 2)

    init {
        subscribeEvent(EventType.GET_OPTIMAL_MODEL)
        subscribeEvent(EventType.CALL_RINGING)
    }
    ...
    override fun handleEvent(type: EventType, bundle: Bundle) {
        when (type) {
            EventType.CALL_RINGING -> {
                ...
                val ratio = getPruningRatio(memoryUsage, cpuUsage, 0.0)
                optimizeModels(ratio)
            }
...
//                        return sttModel;
    ...
    // Executed when statistics does not exist
    public fun getStatistics() {
        val tracker = ModelResourceUsageTracker(applicationContext as Application)
//        tracker.trackIdleResourceUsage()
        val maxSttModelPath = getModel(ModelType.SPEECH_TO_TEXT.value, sttPruningLevels[0])
        val minSttModelPath = getModel(ModelType.SPEECH_TO_TEXT.value, sttPruningLevels[-1])
        val maxDetectionModelPath = getModel(ModelType.PHISHING_DETECTION.value, detectionPruningLevels[0])
        val minDetectionModelPath = getModel(ModelType.PHISHING_DETECTION.value, detectionPruningLevels[-1])
        tracker.trackModelResourceUsage(ModelType.SPEECH_TO_TEXT.value, maxSttModelPath)
        tracker.trackModelResourceUsage(ModelType.SPEECH_TO_TEXT.value, minSttModelPath)
        tracker.trackModelResourceUsage(ModelType.PHISHING_DETECTION.value, maxDetectionModelPath)
        tracker.trackModelResourceUsage(ModelType.PHISHING_DETECTION.value, minDetectionModelPath)
    }

    private fun getModel(modelType: Int, pruningLevel: Int): String {
//        return model path if exists
//        if not exists, download model and copy it, and return the path.
        return ""
    }

    private fun getPruningRatio(memUsage: Double,
                                cpuUsage: Double,
                                storageUsage: Double): Double {
        return 0.5
    }

    private fun optimizeModels(ratio: Double) {
        val modelDir = ""
        val filename = "model_"+(ratio*10).toInt().toString()
//        val filepath =
    }
