package kr.ac.kaist.nmsl.antivp.bcr
...
import kr.ac.kaist.nmsl.antivp.BuildConfig
import kr.ac.kaist.nmsl.antivp.bcr.format.Encoder
import kr.ac.kaist.nmsl.antivp.bcr.format.Format
import kr.ac.kaist.nmsl.antivp.bcr.format.SampleRate
...
import kr.ac.kaist.nmsl.antivp.bcr.Retention
...
class RecorderThread(
    ...
    // Timestamp
    lateinit var callTimestamp: ZonedDateTime
    ...
    // VoicePhishing
    private val recordingIntervalMSec = 5000L
        ...
        synchronized(filenameLock) {
            ...
            filename = buildString {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                    var callDirectionString = ""
                    when (details.callDirection) {
                        Call.Details.DIRECTION_INCOMING -> callDirectionString = "in"
                        Call.Details.DIRECTION_OUTGOING -> callDirectionString = "out"
                        Call.Details.DIRECTION_UNKNOWN -> {}
                    }
                    append("_$callDirectionString")
                    callDirection = callDirectionString
                }
...
//                    }
            ...
            if (success) {
                listener.onRecordingCompleted(this, Uri.EMPTY)
            } else {
                listener.onRecordingFailed(this, errorMsg, Uri.EMPTY)
            }
        }
    }
     ...
     * this scenario, [RecordingEventListener.onRecordingFailed] will be called with a null
     * [Uri].
     */
    fun cancel() {
    ...
    private fun recordUntilCancelled() {
        ...
        val dialogueId = System.currentTimeMillis().toString()
        listener.onRecordingStarted(this, dialogueId)
        ...
        try {
            ...
            try {
                while (!isCancelled) {
                    ...
                    format = savedFormat.first
                    formatParam = savedFormat.second
                    val initialFilename = System.currentTimeMillis().toString()
                    val outputDir = DocumentFile.fromFile(File(FileManager.getInstance().getRootDirectory() + "/calls/$dialogueId"))
                    val outputFile = createFileInDir(outputDir, initialFilename, format.mimeTypeContainer)
                    //val outputFile = createFileInDefaultDir(initialFilename, format.mimeTypeContainer)

                    openFile(outputFile, true).use { pfd ->
                        val container = format.getContainer(pfd.fileDescriptor)

                        try {
                            // audioRecord.format has the detected native sample rate
                            val mediaFormat =
                                format.getMediaFormat(audioRecord.format, formatParam)
                        ...
                        listener.onSavingVoiceFile(this, "/calls/$dialogueId/$initialFilename.wav", dialogueId)
    ...
    private fun encodeLoop(audioRecord: AudioRecord, encoder: Encoder, bufSize: Int) {
        ...
        while (!isCancelled) {
            ...
            if (System.nanoTime() - loopStartTime > recordingIntervalMSec * 1_000_000.0)
                break
        }
    ...
    interface RecordingEventListener {
        /**
         * Called when the recording started successfully. [uri] is the output file.
         */
        fun onRecordingStarted(thread: RecorderThread, dialogueId: String)
        ...
        fun onSavingVoiceFile(thread: RecorderThread, fileName: String, dialogueId: String)
    }
}